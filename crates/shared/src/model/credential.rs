...
impl Credential {
    pub fn fetch(conn: &Connection, id: &CredentialId) -> Result<Credential, anyhow::Error> {
        ...
        let (sql, values) = Query::select()
            .columns([
                ...
                CredentialIden::PublicKey,
                CredentialIden::Aaguid,
                CredentialIden::SignatureCount,
                CredentialIden::CreationDate,
                CredentialIden::LastUsedDate,
                CredentialIden::LastUpdatedDate,
                CredentialIden::Type,
                CredentialIden::Transports,
    ...
    pub fn fetch_passkey(conn: &Connection, id: Uuid) -> Result<WebauthnPasskey, anyhow::Error> {
        let (sql, values) = Query::select()
            .column(CredentialIden::Passkey)
            .from(CredentialIden::Table)
            .and_where(Expr::col(CredentialIden::Id).eq(id))
            .limit(1)
            .build_rusqlite(SqliteQueryBuilder);

        let mut stmt = conn.prepare_cached(&sql)?;
        let passkey = stmt
            .query_row(&*values.as_params(), |r| r.get::<_, Passkey>(0))?;
