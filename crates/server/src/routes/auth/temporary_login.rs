use axum::{extract::Path, http::StatusCode, response::{IntoResponse, Redirect}};
use shared::{
    api::error::{Nothing, ServerError}, bad_request_error, model::{ TemporaryLogin, User}, status_code_error, types::Uuid };
...
pub async fn temporary_login(
    ...
    let id = Uuid::parse(&code)
        .map_err(|_| bad_request_error!("Invalid code ({})", code.as_str()))?;
    ...
    let user = conn.interact(move |conn| {
        ...
        let temporary_login = if let Some(tl) = TemporaryLogin::fetch_maybe(conn, &id)? {
            ...
            if tl.expired() {
                tl.delete(conn)?;
                Err(status_code_error!(StatusCode::GONE, "The code has expired ({})", code.as_str()))
            } else {
                Ok(tl)
            }
        } else {
