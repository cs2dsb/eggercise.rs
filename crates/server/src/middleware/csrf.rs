//! Taken from <https://github.com/daly4/axum-csrf-sync-pattern/tree/feature/tower-sessions>
...
/// Use `CsrfLayer::new()` to provide the middleware and configuration to axum's
/// service stack.
///
/// Use the provided methods to configure details, such as when tokens are
/// regenerated, what request and response headers should be named, and under
/// which key the token should be stored in the session.
#[derive(Clone, Copy, Debug)]
pub struct CsrfLayer {
    /// Configures when tokens are regenerated: Per session, per use or per
    /// request. See [`RegenerateToken`] for details.
    pub regenerate_token: RegenerateToken,

    /// Configures the request header name accepted by the middleware. Defaults
    /// to `"X-CSRF-TOKEN"`. This header is set on your JavaScript or WASM
    /// requests originating from the browser.
    pub request_header: &'static str,

    /// Configures the response header name sent by the middleware. Defaults to
    /// `"X-CSRF-TOKEN"`. This header is received by your JavaScript or WASM
    /// code and its name must be used to extract the token from the HTTP
    /// response.
    pub response_header: &'static str,

    /// Configures the key under which the middleware stores the server-side
    /// token in the session. Defaults to `"_csrf_token"`.
...
impl CsrfLayer {
    /// Create a new CSRF synchronizer token layer to inject into your
    /// middleware stack using [`axum::Router::layer()`].
    pub fn new() -> Self {
        Self::default()
    }

    /// Configure when tokens are regenerated: Per session, per use or per
    /// request. See [`RegenerateToken`] for details.
    ...
    /// Configure a custom request header name accepted by the middleware.
    /// Defaults to `"X-CSRF-TOKEN"`.
    ///
    /// This header is set on your JavaScript or WASM requests originating from
    /// the browser.
    ...
    /// Configure a custom response header name sent by the middleware. Defaults
    /// to `"X-CSRF-TOKEN"`.
    ///
    /// This header is received by your JavaScript or WASM code and its name
    /// must be used to extract the token from the HTTP response.
    ...
    /// Configure a custom key under which the middleware stores the server-side
    /// token in the session. Defaults to `"_csrf_token"`.
...
///
/// You could understand these options as modes to choose a level of paranoia,
/// depending on your application's requirements.
///
/// This paranoia level is a trade-off between ergonomics and security; as more
/// frequent token invalidation requires more overhead for handling and renewing
/// tokens on the client side, as well as retrying requests with a fresh token,
/// should they fail.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
#[allow(clippy::enum_variant_names)]
pub enum RegenerateToken {
    /// Generate one CSRF token per session and use this token until the session
    /// ends.
    ///
    /// This is the default behavior and should work for most applications.
    #[default]
    PerSession,
    /// Regenerate the CSRF token after each use. A "use" describes an unsafe
    /// HTTP method (`POST`, `PUT`, `PATCH`, `DELETE`).
    ///
    /// CSRF tokens are not required for, and thus not invalidated by handling
    /// requests using safe HTTP methods (`HEAD`, `GET`, `OPTIONS`, `TRACE`,
    /// `CONNECT`).
    PerUse,
    /// Regenerate the CSRF token at each request, including safe HTTP methods
    /// (`HEAD`, `GET`, `OPTIONS`, `TRACE`, `CONNECT`).
    ///
    /// This behavior might require elaborate token handling on the client side,
    /// as any concurrent requests mean race conditions from the client's
    /// perspective, and each request's response yields a new token to be
    /// used on the consecutive request.
    PerRequest,
}
...
enum Error {
    ...
    #[error(
        "Session extension missing. Is `tower_sessions::SessionLayer` installed and layered \
         around the `axum_csrf_sync_pattern::CsrfLayer`?"
    )]
    SessionLayerMissing,
...
/// It verifies the CSRF token header on incoming requests, regenerates tokens
/// as configured, and attaches the current token to the outgoing response.
///
/// In detail, this middleware receives a CSRF token as `X-CSRF-TOKEN` (if not
/// custom configured with a different name) HTTP request header value
...
///
/// Optionally regenerates the token from the session after successful
/// verification, to ensure a new token is used for each writing (`POST`, `PUT`,
/// `DELETE`) request. Enable with [`RegenerateToken::PerUse`].
///
/// For maximum security, but severely reduced ergonomics, optionally
/// regenerates the token from the session after each request, to keep the token
/// validity as short as possible. Enable with [`RegenerateToken::PerRequest`].
#[derive(Debug, Clone)]
...
impl<S> CsrfMiddleware<S> {
    /// Create a new middleware from an inner [`tower_service::Service`]
    /// (axum-specific bounds, such as `Infallible` errors apply!) and a
    /// [`CsrfLayer`]. Commonly, the middleware is created by the
    /// [`tower_layer::Layer`] - and never manually.
    ...
    S::Future: Send,
{
    ...
    fn call(&mut self, req: Request<B>) -> Self::Future {
        ...
        Box::pin(async move {
            let session = match req.extensions().get::<Session>().ok_or(Error::SessionLayerMissing)
            {
                Ok(session_handle) => session_handle,
                Err(error) => return Ok(error.into_response()),
            };

            // Extract the CSRF server side token from the session; create a new one if none
            // has been set yet. If the regeneration option is set to "per
            // request", then regenerate the token even if present in the session.
            let mut server_token =
                match session.get::<String>(layer.session_key).await.ok().flatten() {
                    Some(token) => token,
...
mod tests {
    ...
    async fn handler() -> Result<Response, Infallible> {
        Ok((StatusCode::OK, "The default test success response has a body").into_response())
    }
    ...
    async fn post_without_token_fails() {
        let request = Request::builder().method(Method::POST).body(Body::empty()).unwrap();
        let response = app(CsrfLayer::new()).oneshot(request).await.unwrap();
    ...
    async fn session_token_remains_valid() {
        ...
        assert_eq!(BASE64_STANDARD.decode(initial_client_token).unwrap().len(), 32);
    ...
    async fn sends_custom_response_header() {
        ...
        let client_token = response.headers().get("X-Custom-Token-Response-Header").unwrap();
        assert_eq!(BASE64_STANDARD.decode(client_token).unwrap().len(), 32);
    }
    ...
    async fn uses_custom_session_key() {
        ...
        async fn extract_session(session: Session) -> StatusCode {
            ...
            assert_eq!(BASE64_STANDARD.decode(session_csrf_token).unwrap().len(), 32);
            StatusCode::OK
        }
        ...
        let response = app.oneshot(Request::builder().body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn missing_session_layer_error_response() {
        let app = Router::new().route("/", get(handler)).layer(CsrfLayer::new());
