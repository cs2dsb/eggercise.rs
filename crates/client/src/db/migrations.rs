//! TODO: replace
//! This is a simple clone of the features I use from rusqlite_migrations but operating
//! on the sqlite wasm connection we have access to here. The long term goal is to get
//! rusqlite working on wasm and use the same crates as the server but this is sufficient
//! to get working on some client features
...
pub async fn run_migrations(conn: &SqlitePromiser) -> Result<usize, MigrationError> {
    ...
    // conn.exec("DROP TABLE session_exercise; DROP TABLE exercise; DROP TABLE session;").await?;
    let mut version = get_version(conn).await?;
    debug!("Version: {version}");
    
    for (i, m) in MIGRATIONS_DIR
        .dirs()
        .enumerate()
        .skip(version)
    {
        ...
        let up = m
            ...
            .find(|f| f
                .path()
                .ends_with("up.sql"))
            .ok_or(MigrationError::Dir(format!("Migration directory {:?} doesn't contain up.sql", m.path())))?
            .contents_utf8()
            .ok_or(MigrationError::Dir(format!("up.sql in migration directory {:?} could not be read as a utf8 string", m.path())))?;
